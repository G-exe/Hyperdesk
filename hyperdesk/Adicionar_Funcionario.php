			...
			<form action="" id="manage_staff">
				...
				<div class="row">
					<div class="col-md-6 border-right">
						<b class="text-muted">Informações pessoais</b>
						<div class="form-group">
							<label for="" class="control-label">Primeiro Nome</label>
							<input type="text" name="firstname" class="form-control form-control-sm" required value="<?php echo isset($firstname) ? $firstname : '' ?>">
						</div>
						<div class="form-group">
							<label for="" class="control-label">Nome do Meio</label>
							<input type="text" name="middlename" class="form-control form-control-sm"  value="<?php echo isset($middlename) ? $middlename : '' ?>">
						</div>
						<div class="form-group">
							<label for="" class="control-label">Último Nome</label>
					...
					<div class="col-md-6">
						...
						<b class="text-muted">Credenciais do sistema</b>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<small><i><?php echo isset($id) ? "Deixe em branco se não quiser alterar sua senha":'' ?></i></small>
						</div>
						<div class="form-group">
							<label class="label control-label">Confirme a senha</label>
...
<script>
	$('[name="password"],[name="cpass"]').keyup(function(){
		...
		if(cpass == '' ||pass == ''){
			...
			if(cpass == pass){
				$('#pass_match').attr('data-status','1').html('<i class="text-success">Senha correspondida.</i>')
			}else{
				$('#pass_match').attr('data-status','2').html('<i class="text-danger">Senha não corresponde.</i>')
			}
		}
	})
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						location.replace('index.php?page=Lista_de_Funcionarios')
					},750)
				}else if(resp == 2){
